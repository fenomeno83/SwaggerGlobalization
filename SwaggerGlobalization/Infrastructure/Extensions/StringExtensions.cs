...
namespace SwaggerGlobalization.Infrastructure.Extensions
{
    public static class StringExtensions
    {
        ...
        public static string CleanPhoneNumber(this string phone, string prefixToRemove)
        {
            if (string.IsNullOrWhiteSpace(phone))
                return phone.NullableTrim(true);

            phone = phone.Replace("+", "00");

            Regex digitsOnly = new Regex(@"[^\d]");
            var digitOnlyStr = digitsOnly.Replace(phone, string.Empty);

            if (string.IsNullOrWhiteSpace(digitOnlyStr) || string.IsNullOrWhiteSpace(prefixToRemove) || !digitOnlyStr.StartsWith(prefixToRemove.Trim()))
                return digitOnlyStr;

            return Regex.Replace(digitOnlyStr, $"^{prefixToRemove.Trim()}", string.Empty);
        }
